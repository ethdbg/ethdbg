...
class Debugger {
  ...
  async hitBreakpoint(evObj) {
    ...
    this.logger.debug('hit breakpoint');
  }
  ...
  events() {
    ...
    this.vm.on('step', async (eventObj, cb) => {
      ...
      if (c instanceof Contract) {
        ...
        if (isDef(srcLoc.start))
          this.logger.vmDebug(`line range: ${srcLoc.start.line}:${srcLoc.end.line}`);
        if (
          isDef(srcLoc.start) &&
          ((srcLoc.start.line) - srcLoc.end.line === 0) &&
          c.exists(srcLoc.start.line + 1)
        ) {
