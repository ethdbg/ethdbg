...
class Debugger extends EventEmitter {
   ...
   * @author Sean Batzel
   * @param {string} - A string containing the code to be executed.
  ...
  toggleBreakpoint(fp, ln) {
    if (this.cManager.getBySource(fp).includes(ln)) {
      this.cManager.getBySource(fp).remove(ln);
    } else {
      this.cManager.getBySource(fp)[0].add(ln);
    }
    return this;
  }
  ...
  addBreakpoints(breakpoints) {
    if (isDef(breakpoints.source) && isDef(breakpoints.lines)) {
      const contract = this.cManager.getBySource(breakpoints.source);
      breakpoints.lines.forEach((lineNum) => {
        contract.add(lineNum);
      });
  ...
  events() {
    ...
    const hitBreakpoint = async (evObj) => {
      ...
      this.doSomethingIntensive();
      this.logger.vmDebug("HIT A BREAKPOINT");
      return this;
    };
    ...
    const stepHook = async (eventObj, cb) => {
      ...
      if (!isDef(c) &&
        ...
        && !deploying) {
          this.logger.vmDebug('tracking code...');
