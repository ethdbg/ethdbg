...
class GanacheWrapper extends TestRPC {
   ...
   */
  initServerEvents() {
    this.state.blockchain.vm.on('step', async (eventObj, cb) => {
      this.logger.VMDebug(
        `Executed ${eventObj.opcode.name}. PC: ${eventObj.pc}`);
      this.logger.VMDebug("Execution is paused!!!");
      await this.doSomeReallyLongCalc();
      this.logger.VMDebug('waking up');
      cb();
    });
  }
  
  /**
   * used for testing if freezing works
   */
  async doSomeReallyLongCalc() {
