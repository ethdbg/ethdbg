...
/**
 * recurses through a contract grabbing all imports
...
class SolidityWrapper {
  constructor(sourcePath, name) { // eslint-disable-line require-jsdoc
    ...
    const getImports = (source, cwd, sf) => {
      ...
      XRegExp.forEach(source, rx, (match, i, str) => {
        ...
        this.sourceObj[`${cwd}/${match[1]}`] = fs.readFileSync(`${cwd}/${match[1]}`, 'utf8');
        if (this.sourceObj[`${cwd}/${match[1]}`].match(rx).length > 0) {
          let recurse = false;
          let cDir = path.dirname(`${cwd}/${match[1]}`);
          XRegExp.forEach(this.sourceObj[match[1]], rx, (match) => {
            console.log(`${cDir}/${match[1]}`);
            if (!(`${cDir}/${match[1]}` in this.sourceObj)) {
              recurse = true;
            }
          });
          console.log('in assign');
          const cdir = path.dirname(`${cwd}/${match[1]}`);
          console.log(path.dirname(`${cwd}/${match[1]}`));
          if (recurse) {
            _.assign(this.sourceObj,
              getImports(this.sourceObj[match[1]], cdir, match[1]));
          }
        }
      });
      // console.log(sourceObj);
      console.log(Object.keys(this.sourceObj));
