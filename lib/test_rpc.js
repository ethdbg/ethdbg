...
let childProcess = require("child_process"); // will be modifying childProcess, so let
...
class TestRPC {
  ...
  init_events() {
    ...
    this.forked_blockchain.on('message', (msg) => {
       ...
       * |----------------------------|
       * |  recv     <=     1         |
       * |  recv     <=     fork_addr | (Address of forked TestRPC)
       * |  send     =>     1         |
       * |  ready    <=     emitEvent |
       * ------------------------------
      */
  ...
  run_contract() {
    ...
    this.forked_blockchain.on('ready', () => {
