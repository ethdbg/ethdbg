...
class ContractManager {
   ...
   * @param{Function} f - what to do on each contract
   */
  forEach (f) {
    this.cHash.forEach(f);
  }
  /**
  ...
  add(srcFPath) {
    ...
    if (isDef(cmpldSrc.errors) && cmpldSrc.errors.length > 0) {
      throw new Error(`Contract compiled with errors: ${cmpldSrc.errors}`);
    }
  ...
  */
   ...
   * @param{Object} options specifying what to check existence of contract by
   *  - name: Contract Name
   *  - address: contract address
   *  - path: path of contract
   * @return{boolean}
   * @public
   */
  has(options) {
    if (options.address) {
      ...
      this.cHash.forEach((c) => {
        if (c.getPath() === options.path) return true;
      });
    } else {
      throw new Error('must specify one of Name, Address, or path to `has`');
    }
  }
}
