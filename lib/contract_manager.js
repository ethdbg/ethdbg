...
class ContractManager {
  ...
  /**
   * adds a contract by its source file
   * @param{string} srcFPath - sourceFile of contract to add
   * @public
   */
  add(srcFPath) {
   ...
   */
  get(contractName) {
    if (!this.cHash.has(contractName)) {
      throw new Error('Contract does not exist in HashMap');
    } else return this.cHash.get(contractName);
  }
  ...
  /**
   * remove a contract by it's associated source file
