...
class DebugProvider extends Debugger {
  constructor(options) { // eslint-disable-line require-jsdoc
    ...
    this.on(events.isReady, () => {
      if (this.ready) process.stdout.write('true');
      else process.stdout.write('false');
    });
    this.on(events.ready, () => {
      this.readyListeners.forEach(f => f('ready'));
  ...
  /**
   * starts the debugger and initializes all events
   * writes `events.ready` to stdout when TestRPC is up
   * however, not yet in 'debug' mode
   * This will not scan/check for breakpoints, etc.
  ...
  async run() {
    ...
    input.on('data', this.dataIn.bind(this));
    this.on(events.ready, () => {
      this.ready = true;
    });
    return this;
  ...
  dataIn(data) {
    ...
    const todo = deserialize(data);
    this.logger.debug(`emitting event: ${todo[0]}, ${todo[1]}`);
    this.emit(todo[0], todo[1]);
