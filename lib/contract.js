...
class Contract {
  constructor(logger, name, options) {
    ...
    } else if (!chckVar(options.path) && chckVar(options.source)) {
      this.source = options.source;
      this.compiledSource =
        solc.compile(options.source, 1).contracts[this.name];
    } else {
      if (!chckVar(options.path)) {
        throw new Error('\
        One of path, source, or \
        compiledContract must be specified \
    ...
    this.web3; // sometimes used for different web3 contract funcs
  }
  ...
  async getTransaction(provider) {
    if (!chckVar(provider)) throw new Error(`must give a web3 provider`);
    this.web3 = new Web3(new Web3.providers.HttpProvider(provider));
    const proxiedWeb3 = new Proxy(this.web3, proxiedWeb3Handler);
    return await proxiedWeb3.eth.getTransaction(this.getTxHash());
  }
