...
class Contract {
  ...
  async deploy() {
    ...
    const deployFork = fork('./contract_workers');
   ...
   * @return {Contract} testContract - contract to run test funcs
   * @private
   */
  test() {
    ...
    let testContract = this.web3.eth.contract(this.contract.abi)
