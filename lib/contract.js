...
class Contract {
  constructor(logger, name, options) { // eslint-disable-line require-jsdoc
    ...
    if (isDef(options.compiledContract) && isDef(options.AST)) {
      ...
      console.log("cSource.sources = " + cSource.sources['']);
  ...
  async deploy(options) {
    ...
    const deployFork = fork('./lib/contract_workers/deploy');
  ...
  static async getCode(address, provider, options) {
    const getCode = fork('./lib/contract_workers/getCode.js');
  ...
  async runSolidityMethod(name, provider, args) {
    ...
    const runCode = fork('./lib/contract_workers/runSolidityMethod');
