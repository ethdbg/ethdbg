...
var options = {
  ...
  gasLimit: argv.l || argv.gasLimit || 3000000, //3mil gas
...
function fork() {
  ...
  server.listen(options.port, options.hostname, (err, result) => {
    ...
    this.thisLine = new EventEmitter();
    this.nextLine = new EventEmitter();
    ...
    process.send(1);
    let address = (options.hostname || "localhost") + ":" + options.port;
