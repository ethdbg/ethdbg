// mega thanks to @raineorshine at https://github.com/raineorshine/solidity-repl/blob/master/index.js
...
const template = args => `pragma solidity ^0.4.16;
contract ${contractName} {
  function ${main}() public pure constant returns (${args.returnType}) {
    ${args.content}
...
const getAccounts = () => {
  return util.promisify((resolve, reject) => {
...
${source.split('\n').map((line, n) => (n + 1) + '  ' + line).join('\n')}
  ...
  return waterfall([
    ...
    contract => util.promisify(contract[main])()
  ])
}
