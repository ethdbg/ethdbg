...
class SourceMap extends Contract {
   ...
   */
  async initCodeManager(provider) {
    try {
      const tx = await this.getTransaction(provider, this.getLatestTxHash());
  ...
  async getSourceLocationFromPC(pc) {
    const getInstructionIndex = promisify(
      this.codeManager.getInstructionIndex.bind(this.codeManager));
    try {
