...
const {loadWeb3} = require('./remix/init');
const CodeManager = require('./remix/codeManager');
const TraceManager = require('./remix/trace/trace_manager');
...
class SourceMap extends Contract {
   ...
   */
  async initTxManager(provider) {
    const resolveTrace = promisify(this.traceManager.resolveTrace
      .bind(this.traceManager));
    try {
      const tx = await this.getTransaction(provider);
      await resolveTrace(tx);
    } catch (err) {
  ...
  async getSourceLocationFromPC(pc) {
    const getInstructionIndex = promisify(
      this.codeManager.getInstructionIndex.bind(this));
    try {
      const index = await getInstructionIndex(this.getAddress(), pc);
