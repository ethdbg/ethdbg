...
class SourceMap extends Contract {
  ...
  /**
   * useful if there are multiple contracts defined in one file
   */
  getLocationOfContracts() {
    const regx =
      /contract ([a-zA-Z]+) {|contract ([a-zA-Z]+) is [a-zA-Z, ]+ {/gi;
    let source = this.getSource();
    let location = {};
    XRegExp.forEach(source, regx, (match, i, str) => {
      // first capture group, else second
      let contract = isDef(match[1]) ? match[1] : match[2];
      location[contract] = match['index']; //returns byte index of contract def
    });
    return location;
  }

  /**
   * get the range of a defined contract
   * @param{number} defLineNum - line number at which contract is first def
   * get by using getLocationOfContracts
   */
  getRangeOfContract(defLineNum) {
    console.log(this.getSourceMap(defLineNum));
    return this.getSourceMap(defLineNum);
  }
