...
const CodeManager = remixCore.code.CodeManager; const {chckVar} = require('./utils');
...
class SourceMap extends Contract {
   ...
   */
  async initCodeManager(txHash, provider) {
    const prov = chckVar(provider) ? provider : this.provider;
    if (!chckVar(txHash)) throw new Error('txHash must be defined');
    try {
  ...
  async cacheCode() {
    ...
    this.codeManager.codeResolver.cacheExecutingCode(
      this.getAddress(), // used to retrieve code *I think*
      hex,
    );
  }
